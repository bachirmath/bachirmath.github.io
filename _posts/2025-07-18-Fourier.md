...
كما رأينا في النظرية الأولى، تتقارب سلسلة فورير نقطياً نحو الدالة $f(x)$ في النقاط التي تكون فيها الدالة مستمرة، شريطة أن تكون $f$ و $f'$ مستمرتين بالقطعة.

#### التقارب المنتظم (Uniform Convergence)
إذا كانت الدالة $f$ مستمرة ودورية، و $f'$ مستمرة بالقطعة، فإن سلسلة فورير تتقارب منتظماً نحو $f$ على كامل المجال.

#### التقارب في $L^2$ 
**نظرية بارسيفال (Parseval's Theorem)**: إذا كانت $f$ قابلة للتربيع على $[-L,L]$، فإن:

$$\frac{1}{L}\int_{-L}^L |f(x)|^2 dx = \frac{a_0^2}{2} + \sum_{n=1}^{\infty} (a_n^2 + b_n^2)$$

هذا يعني أن سلسلة فورير تتقارب دائماً في $L^2$ نحو الدالة، حتى لو لم تتقارب نقطياً.

### 2. الدوال غير المستمرة - نظرية ديريشليه

**نظرية ديريشليه (Dirichlet's Theorem)**: إذا كانت الدالة $f$ دورية ذات دور $2L$ وتحقق شروط ديريشليه:
- $f$ محدودة على $[-L,L]$
- $f$ لها عدد منته من نقاط عدم الاستمرار
- $f$ لها عدد منته من النقاط الحرجة (القيم العظمى والصغرى المحلية)

فإن سلسلة فورير تتقارب في كل نقطة $x$ نحو:

$$\frac{f(x^+) + f(x^-)}{2}$$

حيث $f(x^+)$ و $f(x^-)$ هما الحدود اليمنى واليسرى للدالة عند النقطة $x$.

**ملاحظة مهمة**: في نقاط الاستمرار، $f(x^+) = f(x^-) = f(x)$، لذلك تتقارب السلسلة نحو $f(x)$. أما في نقاط عدم الاستمرار، فتتقارب نحو متوسط القيمتين اليمنى واليسرى.

### 3. ظاهرة جيبس (Gibbs Phenomenon)

عند تقريب دالة غير مستمرة بسلسلة فورير منتهية (مجموع جزئي)، تظهر ظاهرة جيبس بالقرب من نقاط عدم الاستمرار:

- **التذبذب**: تظهر تذبذبات (oscillations) بالقرب من نقاط القفز
- **الإفراط (Overshoot)**: يتجاوز التقريب القيمة الحقيقية للدالة بحوالي 9% من حجم القفزة
- **عدم الاختفاء**: هذه التذبذبات لا تختفي حتى عند زيادة عدد حدود السلسلة، بل تصبح أكثر تركزاً بالقرب من نقطة عدم الاستمرار

**مثال**: في المثال الأول لدينا (الدالة المربعة)، يمكن ملاحظة ظاهرة جيبس عند النقاط $x = 0, \pm\pi, \pm 2\pi, ..$ حيث تقفز الدالة من 0 إلى 1.

### 4. تمديد الدوال على مجالات منتهية

إذا كانت الدالة $f$ معرفة على مجال $[0,L]$ وأردنا تقريبها بسلسلة مثلثية:

- **لسلسلة $\sin$**: نمدد الدالة بطريقة فردية على $[-L,0]$ بوضع $f(-x) = -f(x)$
- **لسلسلة $\cos$**: نمدد الدالة بطريقة زوجية على $[-L,0]$ بوضع $f(-x) = f(x)$
- **للسلسلة الكاملة**: يمكن تمديد الدالة بأي طريقة على $[-L,0]$ ثم حساب السلسلة الكاملة

كل طريقة تمديد تعطي تقريباً مختلفاً للدالة الأصلية على المجال $[0,L]$.
